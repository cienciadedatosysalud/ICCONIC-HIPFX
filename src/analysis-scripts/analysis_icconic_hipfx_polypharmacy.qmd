...
'Drug_Category' : ['benzodiazepines', 'centrally-acting antihypertensives', 'anticonvulsants', 'anticonvulsants', 'antidepressants', 'antidepressants', 'Antidiabetic agents', 'Antidiabetic agents', 'diuretics', 'diuretics', 'antihistamines', 'H2-receptor antagonists', 'muscle relaxers', 'Nitrates', 'opioids', 'antiparkinson agents', 'Proton Pump Inhibitors', 'antipsychotics', 'antipsychotics', 'other sedatives, anxiolytics and hypnotics', 'steroids', 'steroids', 'steroids'],
'Cate_short_name' : ['BZD', 'CAA', 'CONV', 'CONV', 'DEP', 'DEP', 'DIAB', 'DIAB', 'DIUR', 'DIUR', 'H1_RA', 'H2_RA', 'MUSREL', 'NIT', 'OPI', 'PD', 'PPI', 'PSY', 'PSY', 'SED', 'STER', 'STER', 'STER'],
'Grandparent_Class' : ['nervous_system_agents', 'cardiovascular_agents', 'nervous_system_agents', 'nervous_system_agents', 'nervous_system_agents', 'nervous_system_agents', 'metabolic_agents', 'metabolic_agents', 'cardiovascular_agents', 'cardiovascular_agents', 'respiratory_system', 'gastrointestinal_agents', 'nervous_system_agents', 'cardiovascular_agents', 'nervous_system_agents', 'nervous_system_agents', 'nervous_system_agents', 'nervous_system_agents', 'nervous_system_agents', 'nervous_system_agents', 'hormones', 'hormones', 'hormones'],
'Risk_Class' : ['Fall_increase_high', 'Fall_increase_high', 'Fall_increase_high', 'Fall_increase_moderate', 'Fall_increase_moderate', 'Fall_increase_high', 'Fall_increase_high', 'Bone_mass_decrese_moderate', 'Fall_increase_moderate', 'Fall_increase_high', 'Fall_increase_high', 'Bone_mass_decrese_moderate', 'Fall_increase_moderate', 'Fall_increase_high', 'Fall_increase_moderate', 'Fall_increase_moderate', 'Bone_mass_decrese_high', 'Fall_increase_high', 'Fall_increase_moderate', 'Fall_increase_high', 'Bone_mass_decrese_high', 'Bone_mass_decrese_moderate', 'Fall_increase_moderate']
      }) 
Drug_class_desc  
####(New Updates)#### 
```
...
####(New Updates)####
hip_meds_list = hip_meds_list[hip_meds_list['country_cd'].isin(hip_cohort['country_cd'].unique())]  #Obtain national level crosswalk for country cohort
####(New Updates)####
...
####(New Updates)####
pres_0 = con.execute("SELECT a.*, b. prescription_national_code_cd, b.prescription_start_dt, b.prescription_end_dt FROM hip_cohort_date as a inner join outpatient_prescription as b on a.patient_id = b.patient_id where prescription_start_dt >=pres_window_start and prescription_start_dt<=pres_window_end").fetchdf()
#con.close()
# 9,999 patients left in synthetic data

pres_0['prescription_end_dt'] = pd.to_datetime(pres_0['prescription_end_dt'])
pres_0['prescription_start_dt'] = pd.to_datetime(pres_0['prescription_start_dt']) 

pres_1 = pres_0[['patient_id','pres_window_start','hospital_admission_dt','hospital_discharge_dt','wash_out_end','observation_end','pres_window_end', 'prescription_national_code_cd','prescription_start_dt','prescription_end_dt']]
pres_1 = pres_1.merge(hip_meds_list, how='left', on='prescription_national_code_cd')  # bring drug name and classifications into the dataset
...
####(New Updates)####
pres_all = pres_0[['patient_id','pres_window_start','hospital_admission_dt','hospital_discharge_dt','wash_out_end','observation_end','pres_window_end', 'prescription_national_code_cd','prescription_start_dt','prescription_end_dt']]
pres_all = pres_all[pres_all['patient_id'].isin(hip_polypharm['patient_id'])]
pres_all = pres_all.merge(hip_meds_list, how='left', on='prescription_national_code_cd')
atc_5_full = pres_all[['prescription_national_code_cd', 'prescription_atc_cd']].drop_duplicates()  # bring drug name and classifications into the dataset

atc_5_desc = pres_all[['patient_id', 'pres_window_start', 'hospital_admission_dt', 'wash_out_end', 'observation_end', 'prescription_start_dt', 'prescription_atc_cd']]
atc_5_desc = atc_5_desc.copy()
atc_5_desc.loc[:, 'Med_Time'] = None
...
atc_5_desc_agg_wide.columns = ['patient_id', 'All_Med_Count_Post','All_Med_Count_Pre']
atc_5_desc_agg_wide['All_Med_Count_Pre'] = atc_5_desc_agg_wide['All_Med_Count_Pre'].fillna(0)
atc_5_desc_agg_wide['All_Med_Count_Post'] = atc_5_desc_agg_wide['All_Med_Count_Post'].fillna(0)

ntl_5_desc = pres_all[['patient_id', 'pres_window_start', 'hospital_admission_dt', 'wash_out_end', 'observation_end', 'prescription_start_dt', 'prescription_national_code_cd']]
ntl_5_desc = ntl_5_desc.copy()
ntl_5_desc['Med_Time'] = None
ntl_5_desc.loc[(ntl_5_desc['prescription_start_dt'] >= ntl_5_desc['pres_window_start']) & (ntl_5_desc['prescription_start_dt'] < ntl_5_desc['hospital_admission_dt']), 'Med_Time'] = 'Pre' # Flag prescription during pre admission period
ntl_5_desc.loc[(ntl_5_desc['prescription_start_dt'] > ntl_5_desc['wash_out_end']) & (ntl_5_desc['prescription_start_dt'] <= ntl_5_desc['observation_end']), 'Med_Time'] = 'Post' # Flag prescription during observation period
ntl_5_desc = ntl_5_desc[ntl_5_desc['Med_Time'].isnull() == False] 
ntl_5_desc = ntl_5_desc[['patient_id', 'prescription_national_code_cd', 'Med_Time']].drop_duplicates().reset_index(drop=True)
ntl_5_desc_agg = ntl_5_desc.groupby(['patient_id', 'Med_Time']).agg({'prescription_national_code_cd':'nunique'}).reset_index()
ntl_5_desc_agg_wide = ntl_5_desc_agg.pivot(index='patient_id', columns='Med_Time', values='prescription_national_code_cd').reset_index()
ntl_5_desc_agg_wide.columns = ['patient_id', 'All_Med_Count_Post_NTL','All_Med_Count_Pre_NTL']
ntl_5_desc_agg_wide['All_Med_Count_Pre_NTL'] = ntl_5_desc_agg_wide['All_Med_Count_Pre_NTL'].fillna(0)
ntl_5_desc_agg_wide['All_Med_Count_Post_NTL'] = ntl_5_desc_agg_wide['All_Med_Count_Post_NTL'].fillna(0)
...
hip_polypharm = hip_polypharm.merge(ntl_5_desc_agg_wide, how='left', on='patient_id') # 1006
...
hip_polypharm = hip_polypharm[~hip_polypharm['age_nm'].isnull()]
...
####(New Updates)####
continuous_variable_list = ['age_nm', 'Length_of_stay', 'Comorbidities_Count', 'Cate_short_Count_Pre', 'Cate_short_Count_Post', 'Grandparent_Class_Count_Pre', 'Grandparent_Class_Count_Post', 'Risk_Class_Count_Pre', 'Risk_Class_Count_Post', 'All_Med_Count_Pre', 'All_Med_Count_Post', 'All_Med_Count_Pre_NTL', 'All_Med_Count_Post_NTL']
...
blank_row = pd.DataFrame([['', '', '', '', '', '', '']], columns=['variable', 'Count', 'Mean', 'STD', 'Median', 'Q1', 'Q3'])
...
hip_poly <- hip_poly %>% select(-residence_area_cd)
hip_poly <- na.omit(hip_poly)

hip_poly$exitus_readm_outcome <- as.factor(hip_poly$exitus_readm_outcome)
hip_poly$refracture_outcome <- as.factor(hip_poly$refracture_outcome)

hip_poly$exitus_readm_outcome_num <- as.numeric(hip_poly$exitus_readm_outcome)
hip_poly$refracture_outcome_num <- as.numeric(hip_poly$refracture_outcome)

hip_poly$sex_cd <- as.factor(hip_poly$sex_cd)
hip_poly$previous_hospital_admission_bl <- as.factor(hip_poly$previous_hospital_admission_bl)

columns_to_convert <- c(
  "BZD_patterns", "CAA_patterns", "CONV_patterns", "DEP_patterns",
  "DIAB_patterns", "DIUR_patterns", "H1_RA_patterns", "H2_RA_patterns",
  "MUSREL_patterns", "NIT_patterns", "OPI_patterns", "PD_patterns",
  "PPI_patterns", "PSY_patterns", "SED_patterns", "STER_patterns",
  
  "nervous_system_agents_patterns", "cardiovascular_agents_patterns",
  "metabolic_agents_patterns", "respiratory_system_patterns",
  "gastrointestinal_agents_patterns", "hormones_patterns",
  
  "Fall_increase_moderate_patterns", "Fall_increase_high_patterns",
  "Bone_mass_decrese_high_patterns", "Bone_mass_decrese_moderate_patterns"
)

for (col in columns_to_convert) {
  if (col %in% names(hip_poly)) {
    hip_poly[[col]] <- as.factor(hip_poly[[col]])
  }
}


cate_cols <- c('BZD_patterns', 'CAA_patterns', 'CONV_patterns', 'DEP_patterns', 'DIAB_patterns', 'DIUR_patterns', 'H1_RA_patterns', 'H2_RA_patterns', 'MUSREL_patterns', 'NIT_patterns', 'OPI_patterns', 'PD_patterns', 'PPI_patterns', 'PSY_patterns', 'SED_patterns', 'STER_patterns')
...
formula_cate <- paste(" ~ age_nm + Length_of_stay + sex_cd + previous_hospital_admission_bl + All_Med_Count_Pre + All_Med_Count_Post +", paste(cate_use, collapse = " + "))

cate_cols2 <- c('BZD_patterns','CAA_patterns', 'CONV_patterns', 'DEP_patterns', 'DIAB_patterns', 'DIUR_patterns', 'H1_RA_patterns', 'NIT_patterns', 'OPI_patterns', 'PD_patterns', 'PPI_patterns', 'PSY_patterns', 'SED_patterns', 'STER_patterns')
cate_exist2 <- intersect(cate_cols2, names(hip_poly))
cate_missing2 <- sapply(hip_poly[cate_exist2], function(x) n_distinct(x) == 1)
cate_use2 <- cate_exist2[!cate_missing2]
formula_cate2 <- paste(" ~ age_nm + Length_of_stay + sex_cd + previous_hospital_admission_bl + All_Med_Count_Pre + All_Med_Count_Post +", paste(cate_use2, collapse = " + "))

gp_cols <- c('nervous_system_agents_patterns', 'cardiovascular_agents_patterns', 'metabolic_agents_patterns', 'respiratory_system_patterns', 'gastrointestinal_agents_patterns', 'hormones_patterns')
...
formula_gp <- paste(" ~ age_nm + Length_of_stay + sex_cd + previous_hospital_admission_bl + All_Med_Count_Pre + All_Med_Count_Post +", paste(gp_use, collapse = " + "))
...
formula_risk <- paste(" ~ age_nm + Length_of_stay + sex_cd + previous_hospital_admission_bl + All_Med_Count_Pre + All_Med_Count_Post +", paste(risk_use, collapse = " + "))
...
#### Logistic Model
```{r logit_model, echo=FALSE, warning=FALSE}

####(New Updates)####
m_cate_1ogit_1 <- glm(as.formula(paste("exitus_readm_outcome", formula_cate)), family = binomial(link = "logit"), data = hip_poly)
summary(m_cate_1ogit_1)

m_cate2_1ogit_1 <- glm(as.formula(paste("exitus_readm_outcome", formula_cate2)), family = binomial(link = "logit"), data = hip_poly) 
summary(m_cate2_1ogit_1)

m_gp_1ogit_1 <- glm(as.formula(paste("exitus_readm_outcome", formula_gp)), family = binomial(link = "logit"), data = hip_poly) 
summary(m_gp_1ogit_1)

m_risk_1ogit_1 <- glm(as.formula(paste("exitus_readm_outcome", formula_risk)), family = binomial(link = "logit"), data = hip_poly) 
summary(m_risk_1ogit_1)

m_cate_1ogit_2 <- glm(as.formula(paste("refracture_outcome", formula_cate)), family = binomial(link = "logit"), data = hip_poly) 
summary(m_cate_1ogit_2)

m_gp_1ogit_2 <- glm(as.formula(paste("refracture_outcome", formula_gp)), family = binomial(link = "logit"), data = hip_poly) 
summary(m_gp_1ogit_2)

m_risk_1ogit_2 <- glm(as.formula(paste("refracture_outcome", formula_risk)), family = binomial(link = "logit"), data = hip_poly) 
summary(m_risk_1ogit_2)
...
####(New Updates)####
m_cate_1pm_1 <- lm(as.formula(paste("exitus_readm_outcome_num", formula_cate)), data = hip_poly) 
summary(m_cate_1pm_1)

m_cate2_1pm_1 <- lm(as.formula(paste("exitus_readm_outcome_num", formula_cate2)), data = hip_poly) 
summary(m_cate2_1pm_1)

m_gp_1pm_1 <- lm(as.formula(paste("exitus_readm_outcome_num", formula_gp)), data = hip_poly) 
summary(m_gp_1pm_1)

m_risk_1pm_1 <- lm(as.formula(paste("exitus_readm_outcome_num", formula_risk)), data = hip_poly) 
summary(m_risk_1pm_1)

m_cate_1pm_2 <- lm(as.formula(paste("refracture_outcome_num", formula_cate)), data = hip_poly) 
summary(m_cate_1pm_2)

m_gp_1pm_2 <- lm(as.formula(paste("refracture_outcome_num", formula_gp)), data = hip_poly) 
summary(m_gp_1pm_2)

m_risk_1pm_2 <- lm(as.formula(paste("refracture_outcome_num", formula_risk)), data = hip_poly) 
summary(m_risk_1pm_2)
...
calculate_auc <- function(true_labels, predicted_probs) {
  roc_data <- data.frame(prob = predicted_probs, label = true_labels)
  
  roc_data <- roc_data[order(-roc_data$prob), ]
  
  n_pos <- sum(roc_data$label == 1)
  n_neg <- sum(roc_data$label == 0)
...
extract_model_info <- function(model){
  ...
  auc <- calculate_auc(hip_poly$exitus_readm_outcome, predict(model, type = "response"))
...
outcome_1_list <- list(m_cate_1ogit_1, m_cate2_1ogit_1, m_gp_1ogit_1, m_risk_1ogit_1)
outcome_2_list <- list(m_cate_1ogit_2, m_gp_1ogit_2, m_risk_1ogit_2)
...
for (i in 1:length(outcome_1_list)) {
  ...
  # Write Summary
  writeLines(model_info$summary, file.path(output_folder, paste0("logit_outcome_1_", i, "_summary.txt")))
  
  # Write Odds Ratios
  write.csv(model_info$results, file.path(output_folder, paste0("logit_outcome_1_", i, "_estimates.csv")), row.names = TRUE)
...
for (i in 1:length(outcome_2_list)) {
  ...
  # Write Summary
  writeLines(model_info$summary, file.path(output_folder, paste0("logit_outcome_2_", i, "_summary.txt")))
  
  # Write Odds Ratios
  write.csv(model_info$results, file.path(output_folder, paste0("logit_outcome_2_", i, "_estimates.csv")), row.names = TRUE)
...
outcome_3_list <- list(m_cate_1pm_1, m_cate2_1pm_1, m_gp_1pm_1, m_risk_1pm_1)
outcome_4_list <- list(m_cate_1pm_1, m_gp_1pm_1, m_risk_1pm_1)

for (i in 1:length(outcome_3_list)) {
  
  model_info <- extract_model_info_2(outcome_3_list[[i]])
  
  # Write Summary
  writeLines(model_info$summary, file.path(output_folder, paste0("lpm_outcome_1_", i, "_summary.txt")))
  
  # Write Odds Ratios
  write.csv(model_info$results, file.path(output_folder, paste0("lpm_outcome_1_", i, "_estimates.csv")), row.names = TRUE)
  
}

for (i in 1:length(outcome_4_list)) {
  model_info <- extract_model_info_2(outcome_4_list[[i]])
  
  # Write Summary
  writeLines(model_info$summary, file.path(output_folder, paste0("lpm_outcome_2_", i, "_summary.txt")))
  
  # Write Odds Ratios
  write.csv(model_info$results, file.path(output_folder, paste0("lpm_outcome_2_", i, "_estimates.csv")), row.names = TRUE)
