...
### Filter hospitals with n_patient >=500

variables_in_hospital_filter <- variables_in_hospital %>% filter(n_patient >= 500)
if(descriptive_values$country_cd != 'US'){
  ...
  df_model_filter <- na.omit(df_model)
}
...
#plot(model_exitus_hosp_bl_simple, select = 1, xlab = 'Time from admission to surgery (days)')
...
#plot(model_exitus_7days_bl_simple, select = 1, xlab = 'Time from admission to surgery (days)')
gratia::draw(model_exitus_7days_bl_simple,select = 1,ci_level = 0.95) + 
  geom_hline(yintercept = 0,colour = 'red', lty=2) + 
  labs(title = 'Time from admission to surgery (spline)',x='Time from admission to surgery (days)') +
  theme(panel.background = element_blank(), axis.line = element_line(color = 'black'))
...
#plot(model_exitus_30days_bl_simple, select = 1, xlab = 'Time from admission to surgery (days)')
...
#plot(model_exitus_hosp_bl, select = 1, xlab = 'Time from admission to surgery (days)')
...
# df_var_re <- calculate_ICC(model_exitus_hosp_bl, variable_random_effect = 'hospital_st')
# 
# MOR <- round(exp(sqrt(2*(df_var_re$variance))*qnorm(.75)),5)
# 
# print(paste0('Median odds ratio (MOR): ', MOR))
...
# 
# 
# MOR_w_CI <- calculate_MOR(model_exitus_hosp_bl,var_re = 'hospital_st',n_iter = 1000)
# print(paste0('MOR with bootsrapping: ',round(MOR_w_CI[2],5),' with lower_ci95=',round(MOR_w_CI[1],5),' and upper_ci95=',round(MOR_w_CI[3],5)))
...
#plot(model_exitus_30days_bl, select = 1, xlab = 'Time from admission to surgery (days)')
...
#plot(model_unplanned_7days_bl, select = 1, xlab = 'Time from admission to surgery (days)')
...
#plot(model_unplanned_30days_bl, select = 1, xlab = 'Time from admission to surgery (days)')
