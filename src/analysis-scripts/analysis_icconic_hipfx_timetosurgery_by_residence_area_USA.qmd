...
residence_area_cd_list <- dbGetQuery(conn = con,"select distinct residence_area_cd from main.patient_cohort where residence_area_cd is not null and hospital_st is not null ORDER BY residence_area_cd ASC")
...
for (i in 1:length(residence_area_cd_list)) { 
  ...
  tryCatch(
  {
    ...
    status <- system(paste0("quarto render analysis_icconic_hipfx_timetosurgery.qmd -P state:\"_",residence_area_cd_list[i],"\" --to html --output analysis_icconic_hipfx_timetosurgery_",residence_area_cd_list[i],".html"))
    if(status != 0){
