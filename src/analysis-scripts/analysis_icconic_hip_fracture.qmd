...
df_model <- df_patient_filter %>% dplyr::select(exitus_hosp_bl,exitus_7days_bl,exitus_30days_bl,futime_adm_surg,futime_adm_surg_cd,age_nm,sex_cd,hospital_st,
                      socecon_lvl_cd,surgical_procedure_cd,anesthesia_type_cd,icu_admission_bl,anticoagulant_status_bl,antiaggregant_status_bl,
                      ...
                      traumatic_brain_injury_bl,amputee_bl,year,month,weekday,day_month,age_group,hospital_admission_type_cd,previous_hospital_admission_bl)
...
miss_threshold = 0.2
df_model_aux <- df_model %>% select(!c(age_nm,exitus_hosp_bl,exitus_7days_bl,exitus_30days_bl,futime_adm_surg,futime_adm_surg_cd,hospital_st))
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLM exitus in hospital simple plot
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLM exitus 7 days simple plot
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLM exitus 30 days simple plot
###### exitus 30 days #####
...
###### Model with variable to predict: exitus in hospital
```{r, echo=FALSE,warning=FALSE}
#| label: GLMM exitus in hospital simple plot

###### exitus in hospital #####

### Filter by n_patient >=500
...
# model_exitus_hosp_bl_simple <- glmm::glmm(exitus_hosp_bl ~ futime_adm_surg_cd, random = list(exitus_hosp_bl ~ 1|hospital_st), data = df_model, family.glmm = binomial.glmm())
...
icc <- icc(model_exitus_hosp_bl_simple)
print(paste0('ICC Model with variable to predict: exitus in hospital: ', icc))
#plot(model_exitus_hosp_bl_simple, xlab = 'Time from admission to surgery (days)')
# abline(h = 0.0)
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLMM exitus 7 days simple plot

###### exitus 7 days #####
# model_exitus_7days_bl_simple <- glmm::glmm(exitus_7days_bl ~ futime_adm_surg_cd , random = list(exitus_7days_bl ~ 0 + hospital_st), data = df_model, family.glmm = binomial.glmm(), varcomps.names = c("hospital_st"), m=10)
...
icc <- icc(model_exitus_7days_bl_simple)
print(paste0('ICC Model with variable to predict: exitus in 7 days after surgery: ', icc))
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLMM exitus 30 days simple plot categorised
###### exitus 30 days #####


# model_exitus_30days_bl_simple <- glmm::glmm(exitus_30days_bl ~ futime_adm_surg_cd, random = list(exitus_30days_bl ~ 1|hospital_st), data = df_model, family.glmm = binomial.glmm())
...
icc <- icc(model_exitus_30days_bl_simple)
print(paste0('ICC Model with variable to predict: exitus in 30 days after surgery: ', icc))
...
```{r, echo=FALSE,warning=FALSE}
#| label: GAM exitus in hospital simple plot
...
```{r, echo=FALSE,warning=FALSE}
#| label: GAM exitus 7 days simple plot
...
```{r, echo=FALSE,warning=FALSE}
#| label: GAM exitus 30 days simple plot
###### exitus 30 days #####
...
```{r, echo=FALSE,warning=FALSE}
#| label: GAMM exitus in hospital simple plot

###### exitus in hospital #####


model_exitus_hosp_bl_simple <- mgcv::gamm(exitus_hosp_bl ~ s(futime_adm_surg, bs = 'tp'), data = df_model_filter, random=list(hospital_st=~1),family=binomial(), method = "REML")


summary(model_exitus_hosp_bl_simple$gam)

plot(model_exitus_hosp_bl_simple$gam, select = 1, xlab = 'Time from admission to surgery (days)')
...
```{r, echo=FALSE,warning=FALSE}
#| label: GAMM exitus 7 days simple plot

###### exitus 7 days #####

model_exitus_7days_bl_simple <- mgcv::gamm(exitus_7days_bl ~ s(futime_adm_surg, bs = 'tp'), data = df_model_filter, random=list(hospital_st=~1),family=binomial(), method = "REML")

summary(model_exitus_7days_bl_simple$gam)

plot(model_exitus_7days_bl_simple$gam, select = 1, xlab = 'Time from admission to surgery (days)')
...
```{r, echo=FALSE,warning=FALSE}
#| label: GAMM exitus 30 days simple plot
###### exitus 30 days #####


model_exitus_30days_bl_simple <- mgcv::gamm(exitus_30days_bl ~ s(futime_adm_surg, bs = 'tp'), data = df_model_filter, random=list(hospital_st=~1),family=binomial(), method = "REML")

summary(model_exitus_30days_bl_simple$gam)

plot(model_exitus_30days_bl_simple$gam, select = 1, xlab = 'Time from admission to surgery (days)')
...
Three multilevel GLMM models are built depending on the dependent variable to be predicted (y) as previous section. The most relevant independent variable in this case is (as previous section) the time elapsed between the date of admission and the date of surgery categorized but in this case different patient variables that are considered relevant in the study such as age, sex, comorbidities etc. are introduced into the model.  
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLMM exitus in hospital complex model plot


#formula_model_exitus_hosp_bl <- as.formula(paste0('exitus_hosp_bl ~ futime_adm_surg_cd + ',formula_model))

formula_model_exitus_hosp_bl <- as.formula(paste0('exitus_hosp_bl ~ futime_adm_surg_cd + ',formula_model, ' + (1|hospital_st)'))

# model_exitus_hosp_bl <- glmm::glmm(formula_model_exitus_hosp_bl, data = df_model_filter,random = list(exitus_hosp_bl ~ 1|hospital_st), family.glmm = binomial.glmm(), varcomps.names=c("hospital_st"))

model_exitus_hosp_bl <- glmer(formula_model_exitus_hosp_bl, data = df_model_filter, family = binomial)


summary(model_exitus_hosp_bl)

icc <- icc(model_exitus_hosp_bl)
print(paste0('ICC Model with variable to predict: exitus in hospital: ', icc))
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLMM exitus 7 days complex model plot
...
formula_model_exitus_7days_bl <- as.formula(paste0('exitus_7days_bl ~ futime_adm_surg_cd + ',formula_model, ' + (1|hospital_st)'))
...
model_exitus_7days_bl <- glmer(formula_model_exitus_7days_bl, data = df_model_filter, family = binomial)

summary(model_exitus_7days_bl)
icc <- icc(model_exitus_7days_bl)
print(paste0('ICC Model with variable to predict: exitus in 7 days after surgery: ', icc))
...
```{r, echo=FALSE,warning=FALSE}
#| label: GLMM exitus 30 days complex plot
###### exitus 30 days #####
...
formula_model_exitus_30days_bl <- as.formula(paste0('exitus_30days_bl ~ futime_adm_surg_cd + ',formula_model, ' + (1|hospital_st)'))
...
model_exitus_30days_bl <- glmer(formula_model_exitus_30days_bl, data = df_model_filter, family = binomial)


summary(model_exitus_30days_bl)
icc <- icc(model_exitus_30days_bl)
print(paste0('ICC Model with variable to predict: exitus in 30 days after surgery: ', icc))
